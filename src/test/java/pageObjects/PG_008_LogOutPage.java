...
public class PG_008_LogOutPage {
    ...
    public PG_008_LogOutPage Click_On_Yes() {
        ...
        try {
            WebElement target = yesButton.isDisplayed() ? yesButton : yesButtonXpath;
            target.click();
    ...
    public PG_008_LogOutPage Click_On_No() {
        String methodName = Thread.currentThread().getStackTrace()[1].getMethodName().replace("_", " ");
        try {
            WebElement target = noButton.isDisplayed() ? noButton : noButtonXpath;
            target.click();
            ExtentReportManager.info(methodName);
            TestContext.getLogger().info(methodName);
        } catch (Exception e) {
    ...
    public PG_008_LogOutPage Verify_Logout_Confirmation_Displayed() {
        String methodName = Thread.currentThread().getStackTrace()[1].getMethodName().replace("_", " ");
        try {
            WebElement yes = yesButton.isDisplayed() ? yesButton : yesButtonXpath;
            WebElement no = noButton.isDisplayed() ? noButton : noButtonXpath;
